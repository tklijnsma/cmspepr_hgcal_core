    ...
    if noise_index is not None:
        if noise_index in unique_indices:
            ...
            cluster_index_map = cluster_index_map[
                (unique_indices != noise_index).argsort()
            ]
        else:
...
def mask_fraction_of_noise(
    y: np.array, reduce_fraction: float, noise_index: int = -1, seed: int = None
) -> np.array:
    """
    ...
    n_target_noise = int((1.0 - reduce_fraction) * len(noise_indices))
    ...
    def get(self, i):
        ...
        if i not in self.cache:
            ...
            X, y = make_blobs(
                n_samples=n_hits,
                centers=n_clusters,
                n_features=self.cluster_space_dim,
            ...
            X_bkg = (
                cluster_space_min
                + np.random.rand(n_bkg, self.cluster_space_dim) * cluster_space_width
            )
...
def _taus2021_npzfile_inst_to_torch_data(
    d: np.lib.npyio.NpzFile,
    flip: bool = True,
    sort_by_cluster_index: bool = True,
    npz_path: str = 'mem',
) -> Data:
    """
    ...
    if np.all(y == 0):
        logger.warning('No objects in', npz_path)
...
def taus2021_npz_to_torch_data(
    npzfile: str, flip: bool = True, sort_by_cluster_index: bool = True
):
    return _taus2021_npzfile_inst_to_torch_data(
        np.load(npzfile), flip, sort_by_cluster_index, osp.abspath(npzfile)
    )
